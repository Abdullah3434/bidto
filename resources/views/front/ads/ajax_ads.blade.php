          ...
          <li class="deal_li ada">
                <div class="deal_list_box">
                  ...
                  <div class="deal_list_detail">
                    <h3><a href="javascript:void(0)">{{ucfirst($row->item_name)}}</a></h3>
                    ...
                    @endif  
                    <div class="deal_list_time">{{$row->promotion_end_date_format}}</div></div>
                    <div class="deal_list_price"> 
                      ...
                      @endif
                      @if($row->item_type=='item' && $row->my_bid_position>0)
                        <div class="deal_list_hash_parent">
                            <small>{{Lang::get('label.You')}}</small>
                            @php $color_class = 'tag_grey' ;@endphp
                            @if($row->my_bid_position==1)
                              @php $color_class = 'tag_green' ;@endphp
                            @elseif($row->my_bid_position==2)
                              @php $color_class = 'tag_blue' ;@endphp
                            @elseif($row->my_bid_position==3)
                              @php $color_class = 'tag_yellow' ;@endphp
                            @elseif($row->my_bid_position==4)
                              @php $color_class = 'tag_red' ;@endphp
                            @endif
                            <i class="hash_tag {{$color_class}}">#{{$row->my_bid_position}}</i>
                        </div>
                      @endif
                    </div>
                  </div>
