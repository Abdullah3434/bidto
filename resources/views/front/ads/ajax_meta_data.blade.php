<div class="formRow clearfix">
          ...
          <select class="dropdown_make" id="dropdown_make"  name="ad_make" onchange="change_dropdown_item_make(this)">
            ...
            @if($data->item_makes!=null && $data->item_makes!='')
                ...
                @foreach($data->item_makes as $make_key => $single_make)
                  <option data-value="{{$single_make->id}}"  data-key="{{$make_key}}" data-meta-data="{{json_encode($single_make->models)}}">{!! ucfirst($single_make->make_name) !!}</option>
                @endforeach
            @endif
          </select>
      </div>
    ...
    <div class="formCell col6">
      <div class="selectbox"> 
          <span class="selectbox_span dropdown_model_span">{{Lang::get("label.Model")}}</span>
          <select class="dropdown_model" id="dropdown_model"  name="ad_model" onchange="change_dropdown_item_model(this)">
            <option data-value="" >{{Lang::get("label.Model")}}</option>
          </select>
      </div>
    </div>
    <div class="formCell col6">
      <div class="form_field">
        <input class="ad_max_price" id="ad_max_price"  name="ad_max_price" type="number" placeholder="{{Lang::get('label.Max Price')}}" step="0.01">
      </div>
    </div>
    <div class="formCell col6">
      <div class="form_field">
        <input class="ad_year" id="ad_year"  name="ad_year" type="number" placeholder="{{Lang::get('label.Year')}}">
      </div>
    </div>
